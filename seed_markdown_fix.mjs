...
async function run() {
  ...
  for (const line of lines) {
    ...
    const catId = categories.find(c => c.name === currentCategoryName)?.id;
    if (!catId) continue;
    ...
    if (priceCol.includes('/')) {
      const [full, half] = priceCol.split('/').map(p => parseFloat(p.replace(/[^0-9.]/g, '')));
      fullPrice = full || 0;
      halfPrice = half || 0;
      hasHalfPrice = true;
    } else {
      fullPrice = parseFloat(priceCol.replace(/[^0-9.]/g, '')) || 0;
    }
  ...
  console.log(`🎉 Done! ${menuItems.length} menu items seeded.`);
}
